...
export class SchoolsController {
  ...
  @Get(':schoolId/logo/view') async logoView(@Param('schoolId')id:string,@Res()res:Response){const {file,buffer}=await this.schools.logoBuffer(uuidSchema.parse(id));res.setHeader('Content-Type',file.mime);res.setHeader('Content-Length',buffer.length);res.setHeader('Cache-Control','private, max-age=60');res.setHeader('Content-Disposition','inline');res.end(buffer);}
  ...
  @Get(':schoolId/principal/signature/view') async signatureView(@Param('schoolId')id:string,@Res()res:Response){const {file,buffer}=await this.schools.signatureBuffer(uuidSchema.parse(id));res.setHeader('Content-Type',file.mime);res.setHeader('Content-Length',buffer.length);res.setHeader('Cache-Control','private, max-age=60');res.setHeader('Content-Disposition','inline');res.end(buffer);}
