...
export class PrivateFileService implements OnModuleInit {
  ...
  async onModuleInit(){const exists=await this.client.bucketExists(this.config.minio.bucket).catch(()=>false); if(!exists)await this.client.makeBucket(this.config.minio.bucket).catch(()=>undefined);}
