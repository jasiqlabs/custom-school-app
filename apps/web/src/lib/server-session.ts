...
async function call(path:string){
  try {
    ...
    const cookie=store.getAll().map(c=>`${encodeURIComponent(c.name)}=${encodeURIComponent(c.value)}`).join('; ');
    return await fetch(`${internal}${path}`,{headers:{cookie},cache:'no-store'});
  } catch {
