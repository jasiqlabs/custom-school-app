...
export default function TcUi({ schoolId }: { schoolId: string }) {
  ...
  async function search(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await api<any[]>(`/platform/schools/${schoolId}/students/search?q=${encodeURIComponent(q)}`);
      setResults(res);
    } catch (e: any) {
      setErr(
  ...
  async function issue() {
    ...
    try {
      ...
      setMsg(`TC accepted. UUID: ${tc.tcUuid}`);
      setRows((prev) => [
        {
          id: tc.id,
          tcUuid: tc.tcUuid,
  ...
  async function retry(id: string) {
    try {
      await api(`/platform/schools/${schoolId}/transfer-certificates/${id}/retry-render`, { method: 'POST' });
      load();
    } catch (e: any) {
      setErr(e.message);
    }
  ...
  const ready =
    ...
    school?.principal?.signatureFileId;

  return (
    <div className="container">
      <SchoolNav schoolId={schoolId} />
      <h1>Student Transfer Certificates</h1>
      <div className="card">
        <strong>Readiness:</strong>{' '}
        {ready ? (
          <span className="success">Ready for standard-v1</span>
        ) : (
          <span className="error">Needs ACTIVE school, school logo, principal details and principal signature</span>
        )}
        <div className="muted">School UUID: {schoolId}</div>
      </div>
      {err && (
      ...
      <form className="card row" style={{ marginTop: 16 }} onSubmit={search}>
        <div className="field" style={{ flex: 1 }}>
          <label>Student ID/SR or name</label>
        ...
        <div className="card">
          ...
          {results.map((s) => (
            <button
              ...
              style={{ display: 'block', width: '100%', margin: '8px 0', textAlign: 'left' }}
              onClick={() => setSelected(s)}
            >
              {s.studentCode} — {s.name} — {s.className}/{s.sectionName} — {s.status}
            </button>
          ))}
        </div>
      )}
      {selected && (
        <div className="card" role="dialog">
          <h2>Issue TC</h2>
          <p>
            {selected.studentCode} — {selected.name}
          </p>
          <p>Each generation creates a new immutable TC UUID and freezes the school/student/template snapshot.</p>
          <button disabled={!ready || isGenerating} className="btn btn-primary" onClick={issue}>
      ...
      <div className="card" style={{ marginTop: 16 }}>
        ...
        <table>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id}>
                <td style={{ fontFamily: 'monospace', fontSize: 12 }}>{r.tcUuid}</td>
                <td>
                ...
                <td>
                  {r.status === 'COMPLETED' ? (
                    <button className="btn btn-secondary" onClick={() => download(r.id)}>
                      Download / Print
                    </button>
                  ) : r.status === 'FAILED' ? (
                    <button className="btn btn-secondary" onClick={() => retry(r.id)}>
                      Retry render
                    </button>
                  ) : (
                    'Waiting…'
