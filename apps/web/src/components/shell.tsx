  ...
  return (
    <div>
      <header style={{ background: '#0f172a', color: '#fff', borderBottom: '1px solid #1e293b' }}>
        <div className="container row" style={{ justifyContent: 'space-between', padding: '12px 24px' }}>
          <div className="row" style={{ gap: 14 }}>
            <BackButton variant="navbar" />
            <Link href="/operator" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', color: 'inherit' }}>
              <img
              ...
              />
              <div>
                <strong style={{ fontSize: 14, letterSpacing: '-0.01em' }}>{session?.school?.name || 'School Portal'}</strong>
                <div style={{ fontSize: 11, color: '#34d399' }}>School Operator &bull; GET DIGITAL YOUR SCHOOL</div>
              </div>
            </Link>
          </div>
          <div className="row">
            <span style={{ fontSize: 13, color: '#94a3b8' }}>{session?.operator?.fullName}</span>
            <button
              type="button"
              className="btn btn-secondary"
              disabled={loggingOut}
              onClick={handleLogout}
              style={{ minWidth: 110, fontSize: 13, padding: '6px 14px' }}
            >
